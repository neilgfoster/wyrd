...
exit means a row range in `docs/design/12-oracle-answers.md` disagrees with the maths this feature
committed to in `research.md`.

## Verify the table satisfies `docs/design/07-tables.md`'s own rules
...
every band. Read `docs/design/12-oracle-answers.md`'s table by eye against `docs/design/07-tables.md`'s
row schema to confirm every row carries `range`, `effect`, `description`, and the family's
declared `band` field, and that the key `oracle-answer` matches what `docs/design/07-tables.md`'s
...
grep -rniE '(d&d|dnd|pathfinder|call of cthulhu|blades in the dark|mythic|fate core|apocalypse world)' \
  docs/design/12-oracle-answers.md docs/design/07-tables.md docs/design/01-principles.md
```
...
Expected: exits 0 — `docs/design/12-oracle-answers.md` is reachable from `README.md` via
...
1. Read `docs/design/01-principles.md`'s (amended) obligation clause and confirm it names the same
   class of question `docs/design/12-oracle-answers.md` calls oracle-bound.
