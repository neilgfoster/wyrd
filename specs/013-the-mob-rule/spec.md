...
The mob rule in `design/03-rules.md` §2 was one sentence:
...
which is what [`design/07-tooling.md`](../../design/07-tooling.md) and ADR 0005 exist to prevent.
...
`design/03-rules.md` §2 is rewritten in place, with no "previously we…" note. An ADR records the
...
- **SC-001**: Given any group and any opposing side, a reader determines whether the rule applies
  from `design/03-rules.md` alone, exercising no judgement.
...
- [x] `design/03-rules.md` states the crowd rule with concrete numbers, not prose alone.
