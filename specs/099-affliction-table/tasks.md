...
- [x] T001 Add `AFFLICTION_TABLE` to `engine/wyrd/resolution.py`: the twelve rows of
...
- [x] T002 Add `TRAUMA_FLOOR = 6` and `AFFLICTION_TRAUMA_COST = 6` to
...
- [x] T003 [US1] Add a `terror` entry to `_MECHANICS`/`_PUBLIC_MECHANICS` in
...
- [x] T004 [US1] Add a `trauma-test` entry to `_MECHANICS` in `engine/wyrd/resolution.py`
...
- [x] T005 [US1] Add a `trauma`-crossing branch to `_cascade_from_mutation` in
...
- [x] T006 [P] [US1] `test_trauma_reaching_floor_exactly_stages_no_test` in
  `tests/engine/test_resolution.py` (5 -> 6 via `terror` stages zero `trauma-test` steps).
- [x] T007 [P] [US1] `test_trauma_crossing_past_floor_stages_one_test` in
...
- [x] T008 [P] [US2] `test_multi_point_gain_stages_one_test_per_point` in
...
- [x] T009 [US3] Add `_stage_affliction_roll` to `engine/wyrd/resolution.py`: rolls `1d12`
...
- [x] T010 [US3] Wire a failed `trauma-test` outcome to call `_stage_affliction_roll`; a passed
  outcome stages nothing further (mirrors `_mutate_exposure`'s success/fail branch).
- [x] T011 [P] [US3] `test_failed_trauma_test_stages_affliction_roll_and_loses_six_trauma` in
  `tests/engine/test_resolution.py`.
- [x] T012 [P] [US3] `test_passed_trauma_test_stages_nothing_further` in
  `tests/engine/test_resolution.py`.

## Phase 6: User Story 4 - the affliction table is repeatable (P2)

- [x] T013 [P] [US4] `test_duplicate_affliction_row_is_applied_not_rerolled` in
...
- [x] T014 [P] `test_trauma_never_reaching_floor_stages_no_test` in
  `tests/engine/test_resolution.py`.
- [x] T015 [P] `test_affliction_table_has_twelve_rows_no_severity_field` in
  `tests/engine/test_resolution.py`.
- [x] T016 [P] `test_check_affliction_cadence_unaffected` in `tests/engine/test_resolution.py` (or
...
- [x] T017 `python3 -m ruff check . && python3 -m ruff format --check .`
- [x] T018 `PYTHONPATH=engine python3 -m unittest discover -s tests -p "test_*.py" -q`
