...
## `write_prose(request, paced_candidate, capable_prose, known_entities)`
...
- **Output**: a finished `GenerationResult` (`generation.new_result`'s shape) — `checks: []`,
  `consumed` copied from `selection`.
- **Guarantee**: raises `TypeError` if `capable_prose` is not a `str`. Applies FR-007's labelling
  ...
  `check_entity_membership` recognizes it.

## `run_pipeline(request, *, candidate_pool=None, current=None, haiku_response, capable_prose, known_entities)`
