...
Amend `docs/design/16-session.md`'s companion record so it is explicitly split into a **narrative
...
`docs/design/03-rules.md`'s companion-advancement and succession passage against the completed split
...
named in `docs/design/16-session.md` is a fixed, enumerable set (asserts the exact list this plan
...
and that `docs/design/03-rules.md`'s companion/succession passage does not name a mechanical field
...
capability score (`docs/design/03-rules.md`'s existing "the engine holds no capability score for a
...
**Scale/Scope**: Two existing documents amended (`docs/design/16-session.md` for the two-layer split
and Bond's new positive effect; `docs/design/03-rules.md` for the confirmed advancement/succession
...
- **No capability score for a companion** (`docs/design/03-rules.md`) — Gate: the mechanical layer
  adds no new numeric rating; it names fields the design already defines elsewhere. Verified by
  `tools/check_companion_layers.py` cross-checking the field list against `docs/design/03-rules.md`'s
...
- **Design documents rewritten in place, no changelogs** — `docs/design/16-session.md` and
  `docs/design/03-rules.md` are edited to describe the present state only.
...
have a home (`docs/design/16-session.md`), and this feature refines that document rather than opening
a sixth one, unlike the table-family features which each introduced a new file under a shared
index row. The ADR number is assigned at write time from the next free slot in `docs/adr/`.
