...
- [x] **T001** Write `tools/check_companion_layers.py`: assert the mechanical-layer field set
  named in `design/04-session.md` equals the fixed list `data-model.md` records (`career`,
  `bond`, `taint`, `strain`, `wounds`), no more and no fewer; assert no field appears on both the
  narrative-layer and mechanical-layer lists; assert `design/03-rules.md`'s companion/succession
...
- [x] **T002** Amend `design/04-session.md`'s companion section:
...
- [x] **T003** Amend `design/03-rules.md`'s companion-and-succession passage: add the one-line
  cross-reference to the two-layer terminology now defined in `design/04-session.md`, and confirm
...
- [x] **T004** Write `design/adr/0034-bond-is-the-positive-party-track.md`: records the rejected
