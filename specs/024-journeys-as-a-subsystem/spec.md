...
- Q: How is a hazard's per-leg trigger chance determined? → A: Mirrors the Threat activation
  roll (`doc/design/18-campaign.md`) — a per-journey hazard rating × 10, rolled once per leg.
- Q: What decides whether a leg is played (a beat) versus summarised? → A: Author-declared,
  ...
  (`doc/design/28-arcs-and-beats.md`) — set when the journey/leg is authored or converted.
...
1. **Given** a journey between two places with a known distance and pace, **When** the GM
   ...
   `doc/design/18-campaign.md`'s existing split.
...
`doc/design/26-authoring-a-setting.md` "a setting may never add a mechanism" rule is meant to
...
1. **Given** a setting that does not configure journeys, **When** the character travels in
   play, **Then** travel is narrated exactly as `doc/design/18-campaign.md` already describes,
...
1. **Given** a stub arc tagged as a journey, **When** it is converted, **Then** its children
   are legs expressed as ordinary beats or nested arcs (per `doc/design/28-arcs-and-beats.md`),
...
- What happens when a journey's hazard table has no entries (a setting configures roles and
  ...
  `doc/design/18-campaign.md` — it simply never rolls on an empty table.
...
- **FR-001**: The engine MUST define a journey as a structural extension of the existing
  arc/beat shape (`doc/design/28-arcs-and-beats.md`) — a sequence of legs, each an arc or a beat
  — not a new content type outside that tree.
- **FR-002**: A journey MUST resolve each leg through one of the two mechanisms
  `doc/design/18-campaign.md` already defines: **played** (a beat, run through the core roll) or
...
- **FR-004**: Any hazard that triggers during a journey MUST resolve through the core
  percentile roll (`doc/design/03-rules.md`), using an existing skill and difficulty — a journey
...
- **FR-004a**: Each journey MUST carry a hazard rating; each leg rolls `d100` once against
  ...
  (`doc/design/18-campaign.md`) rather than introducing a second per-leg-chance formula.
- **FR-004b**: Each leg MUST declare its own resolution mode (`played` or `summarised`) using
  the existing `mode:` field beats already carry (`doc/design/28-arcs-and-beats.md`); the engine
  does not choose a leg's mode at runtime.
- **FR-005**: Supply and encumbrance consequences arising from a journey MUST use the
  existing material-economy abstraction (`doc/design/04-the-character.md` /
...
- **FR-006**: The subsystem MUST be fully optional per setting: a setting that configures no
  ...
  `doc/design/18-campaign.md`), and every journey-specific configuration field MUST have a
...
- **FR-008**: The elapsed-time and expected-value machinery a journey uses for its
  summarised legs MUST be the same mechanism `doc/design/18-campaign.md` already defines
...
- **SC-002**: A setting that does not configure journeys shows zero behavioural difference
  ...
  `doc/design/18-campaign.md`'s existing elapsed-time section and confirming nothing there
...
- A journey is authored the same way any other arc is: it may start as a stub and be
  converted lazily (`doc/design/28-arcs-and-beats.md`); this feature defines the shape, not a
  populated example.
