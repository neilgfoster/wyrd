...
`design/13-authoring-a-setting.md` "a setting may never add a mechanism" rule is meant to
...
1. **Given** a setting that does not configure journeys, **When** the character travels in
   play, **Then** travel is narrated exactly as `design/05-campaign.md` already describes,
...
1. **Given** a stub arc tagged as a journey, **When** it is converted, **Then** its children
   are legs expressed as ordinary beats or nested arcs (per `design/15-arcs-and-beats.md`),
...
- What happens when a journey's hazard table has no entries (a setting configures roles and
  ...
  `design/05-campaign.md` — it simply never rolls on an empty table.
...
- **FR-001**: The engine MUST define a journey as a structural extension of the existing
  arc/beat shape (`design/15-arcs-and-beats.md`) — a sequence of legs, each an arc or a beat
  — not a new content type outside that tree.
- **FR-002**: A journey MUST resolve each leg through one of the two mechanisms
  `design/05-campaign.md` already defines: **played** (a beat, run through the core roll) or
...
- **FR-006**: The subsystem MUST be fully optional per setting: a setting that configures no
  ...
  `design/05-campaign.md`), and every journey-specific configuration field MUST have a
...
- **FR-008**: The elapsed-time and expected-value machinery a journey uses for its
  summarised legs MUST be the same mechanism `design/05-campaign.md` already defines
...
- **SC-002**: A setting that does not configure journeys shows zero behavioural difference
  ...
  `design/05-campaign.md`'s existing elapsed-time section and confirming nothing there
...
- A journey is authored the same way any other arc is: it may start as a stub and be
  converted lazily (`design/15-arcs-and-beats.md`); this feature defines the shape, not a
  populated example.
