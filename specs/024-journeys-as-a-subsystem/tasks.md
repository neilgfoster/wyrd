...
- [X] T001 Write `docs/design/20-journeys.md`: define a journey as a `scale: journey` arc
...
- [X] T002 In `17-journeys.md`, state the per-leg hazard roll exactly: `d100 ≤ hazard_rating ×
  ...
  FR-004a]
- [X] T003 In `17-journeys.md`, state that a leg's resolution mode (`played`/`summarised`) is
  ...
  FR-004b, FR-008]
- [X] T004 In `17-journeys.md`, state that a journey may end before its declared distance is
  ...
  journey. [FR-007]
- [X] T005 In `17-journeys.md`, state that supply/harm/Standing consequences from a hazard or a
...
- [X] T006 In `17-journeys.md`, state the passage-through-a-Threat's-reach case: travelling
...
- [X] T007 In `docs/design/19-campaign.md`'s elapsed-time section, add one cross-reference line to
  `17-journeys.md` for the played-journey case, changing no other prose in that section. [FR-002,
  FR-006, SC-002]
- [X] T008 In `docs/design/18-arcs-and-beats.md`, add one cross-reference noting `scale: journey` as
  a recognised arc scale, pointing to `17-journeys.md` for its fields. [FR-001, SC-001]
- [X] T009 In `docs/design/24-authoring-a-setting.md`'s worked example, update the "Journeys as a
...
- [X] T010 Add `17-journeys` to `README.md`'s design table, following the existing row
  convention. [reachability — CLAUDE.md's document-graph rule]
- [X] T011 Check `settings.yaml`'s `tor` entry's `note:` against the finished
...
- [X] T012 Read `17-journeys.md` cold, with only `05-campaign.md`, `15-arcs-and-beats.md`, and
...
- [X] T014 Diff `docs/design/19-campaign.md`'s elapsed-time section before/after T007 and confirm the
...
- [X] T017 Run `python3 tools/backlog.py check` — must pass.
