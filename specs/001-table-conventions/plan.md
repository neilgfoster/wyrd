...
Create `docs/design/04-tables.md`: the engine's table index and the shared conventions every table
...
nothing new — the override contract is the one `docs/design/24-authoring-a-setting.md` already draws, and
the version pin reuses the four versions `docs/design/22-state.md` already defines rather than
introducing a fifth.

Alongside the new document, four existing design documents gain links to it and one stale list is
corrected (`docs/design/27-tooling.md:84` omits afflictions). One ADR records the decision that a real
...
| Rules changes are forward-only | ADR-adjacent, `docs/design/29-evolution.md` | **Pass** — R7 states a table change is tuning or additive, never retroactive |
...
provenance shape `docs/design/29-evolution.md:105` already defines rather than creating a new one.
...
**Structure Decision**: The new document is `docs/design/04-tables.md`, sitting between `03-rules.md`
...
3. **`docs/design/27-tooling.md:84` is a real stale list, not a typo.** It omits afflictions while
