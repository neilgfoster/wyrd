...
**Rationale**: `docs/design/03-rules.md:114` already commits to `1d6 + points below zero` for criticals.
...
which is what `docs/design/03-rules.md:115` already means by "high results are lethal": the top row is
...
**Rationale**: `docs/design/03-rules.md:205` has a transformation consume Taint equal to its severity, so
...
family's declared exhaustion outcome — for transformations, `docs/design/03-rules.md:213` already supplies
...
`docs/design/02-architecture.md:91` says `tables/` without a filename convention; this fills that in
...
**Rationale**: `docs/README.md`'s two-part test. A real alternative was rejected — one universal
