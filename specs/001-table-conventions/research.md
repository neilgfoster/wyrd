...
Phase 0 output. Each decision below is settled here so `doc/design/07-tables.md` can state it as
...
**Rationale**: `doc/design/03-rules.md:114` already commits to `1d6 + points below zero` for criticals.
...
which is what `doc/design/03-rules.md:115` already means by "high results are lethal": the top row is
...
- *A single free-text row.* Simplest to author. Rejected: the engine could not apply the result, so
  every table would need an AI to interpret it — the exact inference `doc/design/20-tooling.md` and
...
**Rationale**: `doc/design/03-rules.md:205` has a transformation consume Taint equal to its severity, so
...
family's declared exhaustion outcome — for transformations, `doc/design/03-rules.md:213` already supplies
...
**Rationale**: `doc/design/26-authoring-a-setting.md:157` already publishes
...
`doc/design/02-architecture.md:91` says `tables/` without a filename convention; this fills that in
...
the engine that produced it (`doc/design/22-evolution.md:105`). The outcome additionally records the
table key it rolled on. No per-table version is introduced.

**Settled by**: operator, clarification session 2026-08-22.

**Rationale**: `doc/design/19-state.md:29` is explicit that four things carry versions and enumerates
...
**Change class**: a table change is *tuning* under `doc/design/22-evolution.md:37` when it alters
...
**Rationale**: `doc/design/26-authoring-a-setting.md` already draws exactly this line: content is a
...
1. The key is one the engine publishes; an unknown key is a load error
   (`doc/design/26-authoring-a-setting.md`: the overridable set is closed).
...
**Renames**: presentation-only, per `doc/design/26-authoring-a-setting.md` and ADR 0004. A rename
...
**Rationale**: `doc/README.md`'s two-part test. A real alternative was rejected — one universal
...
three are stated in `doc/design/07-tables.md` itself and their rejected alternatives are recorded here
in this committed spec — which is what `CLAUDE.md` says a spec is for. Writing four records where
one is load-bearing is the noise `doc/README.md` warns turns records into something nobody reads.
...
|---|---|---|---|
| `doc/design/20-tooling.md` | 84 | Lists "criticals, aftermath, transformations, oracles" — omits afflictions, which `doc/design/02-architecture.md:91` includes and `doc/design/03-rules.md:229` requires | Add afflictions, so the two lists match |
| `doc/design/02-architecture.md` | 91 | Names the families but nothing points to where they are defined | Link to `doc/design/07-tables.md` |
| `doc/design/03-rules.md` | 115, 123, 205, 229 | Names four table families without resolving any of them | Link each to `doc/design/07-tables.md` |
| `doc/design/26-authoring-a-setting.md` | 157 | The `overrides.tables:` example is the only statement of the contract and states none of its rules | Link to the override contract in `doc/design/07-tables.md` |
| `doc/README.md` | Index | Will not list the new ADR | Add its row |

`doc/design/22-evolution.md` and `doc/design/19-state.md` need no change: the conventions were chosen to fit
