...
```bash
test -f design/03a-tables.md && echo present
grep -rn "03a-tables" design/ README.md
```

Expected: the file is present, and `design/02-architecture.md`, `design/03-rules.md`,
`design/07-tooling.md` and `design/13-authoring-a-setting.md` each link to it. A reader arriving at
...
Read `design/03a-tables.md` and confirm each of these is stated, not implied:
...
```bash
grep -n "tables/" design/02-architecture.md design/07-tooling.md
```

Expected: both list the same five families — criticals, aftermath, transformations, afflictions,
oracles — and all five appear in the index in `design/03a-tables.md`. The two sets match exactly
(SC-002). `design/07-tooling.md:84` omitted afflictions before this change; confirm it no longer
does.

## 4. Every rule's table reference resolves

```bash
grep -n -iE "table" design/03-rules.md
```
...
Inspect the index table in `design/03a-tables.md`. Expected: adding a sixth family is one new row
...
```bash
grep -n -iE "version|schema_version" design/03a-tables.md
```

Expected: the document introduces no version, no storage location and no override key beyond those
`design/06-state.md` and `design/13-authoring-a-setting.md` already define (SC-007). Specifically:
...
Read `design/03a-tables.md` against `design/02-architecture.md`, `design/07-tooling.md`,
`design/09-evolution.md` and `design/13-authoring-a-setting.md`. Expected: no statement in the new
