...
(`docs/design/27-tooling.md` describes it as future work). Verification is the mechanical grep-and-read
...
- [x] T001 Create `docs/design/04-tables.md` with its title, a one-paragraph statement of what the
...
- [x] T002 Correct the stale family list at `docs/design/27-tooling.md:84`: it reads "criticals,
      aftermath, transformations, oracles" and omits afflictions, which `docs/design/02-architecture.md:91`
      includes and `docs/design/03-rules.md:229` requires. Add afflictions so the two lists match
      (research.md R10, FR-015). This is fault class 3 — both documents read as coherent alone.
- [x] T003 Confirm by reading `docs/design/02-architecture.md:91`, the corrected `docs/design/27-tooling.md:84`
      and every table reference in `docs/design/03-rules.md` that the family set is exactly five:
...
- [x] T004 Write the **index** section of `docs/design/04-tables.md`: one row per family, each with its
...
- [x] T007 Write the **naming and file layout** paragraph: keys are lowercase and hyphenated,
      ...
      at `docs/design/24-authoring-a-setting.md:157`, which is the only existing evidence of either
...
- [x] T009 Write the **override contract** section of `docs/design/04-tables.md`: a setting replaces a
...
- [x] T012 [P] Amend `docs/design/24-authoring-a-setting.md` near line 157 to link the
      `overrides.tables:` example to the contract in `docs/design/04-tables.md`, so the one published
...
- [x] T013 Write the **versioning** section of `docs/design/04-tables.md`: a table is pinned by the
      version stamps that already exist — engine and setting versions in `chronicle.yaml`, the
      engine stamped on every outcome (`docs/design/29-evolution.md:105`) — plus the table key recorded
...
- [x] T014 State that a table change is *tuning* when it alters ranges, effects or numbers within an
      ...
      `docs/design/29-evolution.md:37`).
...
- [x] T015 [P] Amend `docs/design/02-architecture.md:91` to link the `tables/` bullet to
      `docs/design/04-tables.md` (FR-015, R10).
- [x] T016 [P] Amend `docs/design/27-tooling.md:84` to link the `tables.py` line to
      `docs/design/04-tables.md` — the conventions are what that module will implement (FR-015, R10).
- [x] T017 [P] Amend `docs/design/03-rules.md` at each of lines 115, 123, 205 and 229 to link the named
      table to `docs/design/04-tables.md`, so every table reference in the ruleset resolves (FR-015,
      SC-005, R10).
- [x] T018 Write `docs/adr/0008-tables-declare-their-own-roll.md`: dated, accepted, recording that
      ...
      family. It earns a record on `docs/README.md`'s two-part test — a workable smaller engine
      was rejected, and "why does every table roll differently?" is the first question a reader asks
      (research.md R9).
- [x] T019 Add the ADR 0008 row to the index in `docs/README.md` (FR-017).
- [x] T020 Run every mechanical check in [quickstart.md](./quickstart.md) §1, §3, §4, §6, §7 and
      ...
      a script, check it.
- [x] T021 Read `docs/design/04-tables.md` against `docs/design/02-architecture.md`,
      `docs/design/27-tooling.md`, `docs/design/29-evolution.md` and `docs/design/24-authoring-a-setting.md` for
...
- [x] T023 Confirm the index is genuinely append-only (quickstart §5, SC-003): the families are
      enumerated in exactly one place in `docs/design/04-tables.md`. If prose elsewhere in the document
...
- US1, US2 and US3 all write into `docs/design/04-tables.md`, so their tasks are **not** parallel with
