...
(`design/07-tooling.md` describes it as future work). Verification is the mechanical grep-and-read
...
- [x] T001 Create `design/03a-tables.md` with its title, a one-paragraph statement of what the
...
- [x] T002 Correct the stale family list at `design/07-tooling.md:84`: it reads "criticals,
      aftermath, transformations, oracles" and omits afflictions, which `design/02-architecture.md:91`
      includes and `design/03-rules.md:229` requires. Add afflictions so the two lists match
      (research.md R10, FR-015). This is fault class 3 — both documents read as coherent alone.
- [x] T003 Confirm by reading `design/02-architecture.md:91`, the corrected `design/07-tooling.md:84`
      and every table reference in `design/03-rules.md` that the family set is exactly five:
...
- [x] T007 Write the **naming and file layout** paragraph: keys are lowercase and hyphenated,
      ...
      at `design/13-authoring-a-setting.md:157`, which is the only existing evidence of either
...
- [x] T009 Write the **override contract** section of `design/03a-tables.md`: a setting replaces a
...
- [x] T012 [P] Amend `design/13-authoring-a-setting.md` near line 157 to link the
      `overrides.tables:` example to the contract in `design/03a-tables.md`, so the one published
...
- [x] T013 Write the **versioning** section of `design/03a-tables.md`: a table is pinned by the
      version stamps that already exist — engine and setting versions in `chronicle.yaml`, the
      engine stamped on every outcome (`design/09-evolution.md:105`) — plus the table key recorded
...
- [x] T015 [P] Amend `design/02-architecture.md:91` to link the `tables/` bullet to
      `design/03a-tables.md` (FR-015, R10).
- [x] T016 [P] Amend `design/07-tooling.md:84` to link the `tables.py` line to
      `design/03a-tables.md` — the conventions are what that module will implement (FR-015, R10).
- [x] T017 [P] Amend `design/03-rules.md` at each of lines 115, 123, 205 and 229 to link the named
      table to `design/03a-tables.md`, so every table reference in the ruleset resolves (FR-015,
      SC-005, R10).
- [x] T018 Write `design/adr/0008-tables-declare-their-own-roll.md`: dated, accepted, recording that
      ...
      family. It earns a record on `design/README.md`'s two-part test — a workable smaller engine
      was rejected, and "why does every table roll differently?" is the first question a reader asks
      (research.md R9).
- [x] T019 Add the ADR 0008 row to the index in `design/README.md` (FR-017).
- [x] T020 Run every mechanical check in [quickstart.md](./quickstart.md) §1, §3, §4, §6, §7 and
      ...
      a script, check it.
- [x] T021 Read `design/03a-tables.md` against `design/02-architecture.md`,
      `design/07-tooling.md`, `design/09-evolution.md` and `design/13-authoring-a-setting.md` for
...
- [x] T023 Confirm the index is genuinely append-only (quickstart §5, SC-003): the families are
      enumerated in exactly one place in `design/03a-tables.md`. If prose elsewhere in the document
...
- US1, US2 and US3 all write into `design/03a-tables.md`, so their tasks are **not** parallel with
