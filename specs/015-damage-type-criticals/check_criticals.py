...
docs/design/05-criticals.md publishes is asserted here, so a change to either fails loudly rather
...
2. A telling blow doubles the damage rolled, before armour subtracts (docs/design/03-rules.md
   section 2).
3. A critical happens when damage takes a combatant below 0 Stamina, and reads 1d6 + points
   below zero (docs/design/03-rules.md section 2, docs/design/04-tables.md).
4. Aftermath reads d100 + 5 x points below zero, and death begins at 111
   (docs/design/06-aftermath.md).
5. A starting character has Stamina 6, 7 after a completed career
   (docs/design/11-character-creation.md, docs/design/03-rules.md section 5).
...
CAREER_STAMINA = 7
CRITICAL_DIE = 6  # 1d6
AFTERMATH_DEATH_FROM = 111  # docs/design/06-aftermath.md
...
# Every figure docs/design/05-criticals.md publishes, asserted below. A table is where staleness
...
@cache
def damage_through(
    weapon: tuple[int, ...], armour: tuple[int, ...], telling: bool
...
def aftermath_death_chance(below: int) -> Fraction:
    """P(d100 + 5*below >= 111), the death rows as docs/design/06-aftermath.md publishes them."""
...
def main() -> int:
    ...
    median = min(
        b
        for b in sorted(mod_dist)
        if sum((p for k, p in mod_dist.items() if k <= b), Fraction(0)) >= Fraction(1, 2)
    )
    ...
    check(
        abs(float(tail_15) * 100 - PUBLISHED_MODIFIER_AT_LEAST_15) < TOLERANCE,
    ...
    for table, rows in TABLES.items():
        ...
        for i, ((lo, hi), _, _) in enumerate(rows):
            if i + 1 < len(rows):
                check(
                    hi is not None and rows[i + 1][0][0] == hi + 1,
        ...
        for total in range(LOWEST_TOTAL, CRITICAL_DIE + max_below + 50):
            hits_rows = [
                k for (lo, hi), k, _ in rows if total >= lo and (hi is None or total <= hi)
            ]
            check(
                len(hits_rows) == 1,
        ...
        for _, key, effect in rows:
            for name in effect:
                check(
                    name in LEGAL_EFFECTS,
                    f"{table} row {key} names effect '{name}', which is not one the engine knows.",
                )
            check(
                "trauma" not in effect,
                f"{table} row {key} charges Trauma, which docs/design/03-rules.md section 5 "
                "already charges once per critical taken.",
            )
    ...
    check(
        max(weights, key=lambda t: weights[t][0]) == "critical-blunt",
    ...
    check(
        abs(float(aftermath_alone) * 100 - PUBLISHED_AFTERMATH_ALONE) < TOLERANCE,
    ...
    # docs/design/06-aftermath.md publishes 23%, UNWEIGHTED across drops of one to twelve. This
    ...
    check(
        abs(float(unweighted) - 0.23) < 0.01,
        f"Unweighted Aftermath death computes to {pct(unweighted)}, against the 23% "
        "docs/design/06-aftermath.md publishes. One of the two models is wrong.",
    )
    check(
        aftermath_alone < unweighted,
    ...
    print("=" * 78)
    earliest_mortal = min(
        lo for rows in TABLES.values() for (lo, hi), _, e in rows if e.get("mortal")
    )
    ...
    check(
        earliest_mortal == 19,
        f"The earliest mortal row begins at {earliest_mortal}, not the 19 the document publishes.",
    )
    for table in TABLES:
        worst_at_one = row_for(table, CRITICAL_DIE + 1)
        print(f"  {table:<20} dropped by 1, worst possible: {worst_at_one[0]}")
        check(
            not worst_at_one[1].get("mortal"),
