...
(`design/03-rules.md`), so every one of them is fought a roll at a time.
...
cost him 1 Trauma, by `design/03-rules.md` §5 — the table charged him nothing.
...
2. **The telling blow is where the modifier comes from.** Every large modifier in this fight came
   ...
   `design/03-rules.md` had not said it in so many words. It says it now.
...
5. **One question this fight raised belongs to another issue.** When the acting side succeeds and
   the resisting side *fails*, `design/03-rules.md` does not say what the margin is — and the
