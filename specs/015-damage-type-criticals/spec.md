...
types and define one critical table per type in `doc/design/08-criticals.md`, on the shared
`1d6 + points below zero` ladder.

## Why this exists

[`doc/design/03-rules.md`](../../doc/design/03-rules.md) has told the GM to roll `1d6 + points below zero`
...
[`doc/design/26-authoring-a-setting.md`](../../doc/design/26-authoring-a-setting.md), and "he is Blunt 5" in
[`doc/design/23-diegesis.md`](../../doc/design/23-diegesis.md).
...
and the effect are all determinable from `doc/design/08-criticals.md` alone, with no judgement call.
...
3. **Given** a critical is taken, **When** its effect is applied, **Then** 1 Trauma is charged once,
   by the rule in `doc/design/03-rules.md` §5, and no row charges it again.
...
2. **Given** a setting that replaces a critical table's rows, **When** the setting loads, **Then**
   the replacement is checked against the rules in `doc/design/07-tables.md` and an unknown key is a
   load error.
...
2. **Given** a spent Fate point, **When** the composition rule would end in death, **Then** Fate
   answers it by the mechanism `doc/design/09-aftermath.md` already publishes, unchanged.

### Edge Cases

- **The smallest possible critical.** One point below zero on a roll of 1 — the lowest total the
  family can produce. It must be the first row's lower bound, per `doc/design/07-tables.md`.
...
- **FR-002**: Every damage type MUST have exactly one critical table, defined in
  `doc/design/08-criticals.md`, one table per key.
...
- **FR-004**: Every row MUST carry range, key, effect and description, per the row schema in
  `doc/design/07-tables.md`. The family MUST declare whether it carries any extra field, and MUST NOT
...
- **FR-006**: The family MUST declare itself repeatable or unique per character, and MUST answer
  the consequent obligations `doc/design/07-tables.md` places on that declaration.
...
- **FR-009**: The tables MUST NOT charge Trauma. `doc/design/03-rules.md` §5 already charges 1 per
...
- **FR-011**: `doc/design/07-tables.md`'s index row for Criticals MUST be updated to link the file, and
  `doc/design/03-rules.md` and `doc/design/26-authoring-a-setting.md` MUST be updated wherever the
...
- **Wound record**: the state entry a lasting effect writes, already defined in
  `doc/design/09-aftermath.md` and `doc/design/19-state.md`.

## Success Criteria *(mandatory)*

- **SC-001**: A GM given a damage type, a Stamina value and a damage roll can resolve a critical
  from `doc/design/08-criticals.md` alone, with no judgement call and no second document.
...
- **SC-006**: `doc/design/03-rules.md`, `doc/design/07-tables.md`, `doc/design/09-aftermath.md` and
  `doc/design/26-authoring-a-setting.md` agree with the new document on the damage types, the roll, and
...
- **#15 has landed**, so `doc/design/07-tables.md` is the authority on conventions and this feature
  conforms to it rather than restating it.
- **#16 has landed**, so `doc/design/09-aftermath.md` exists and the composition rule is written
