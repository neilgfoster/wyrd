...
Enumerate four damage types, write one critical table for each in `design/03a-1-criticals.md` on the
...
6. **Agreement with the figures merged issues computed** — 1.56 through modest armour and 4.5 hits
   ...
   (`design/03a-2-aftermath.md`). A private damage model would make everything here internally tidy
   and wrong.
...
| Document | Change |
|---|---|
| `design/03a-1-criticals.md` | **new** — the four types, the four tables, the composition with Aftermath, what a setting may replace |
| `design/03a-tables.md` | the Criticals index row gains its link; the `critical-slashing` example stays true |
| `design/03-rules.md` §2 | the critical rule points at the new file and names the types |
| `design/03a-2-aftermath.md` | the boundary table gains the mortal-blow row; the death-row section gains its mirror |
| `design/13-authoring-a-setting.md` | the override example verified against the published keys |
| `design/README.md` | the new document linked, so `tools/check_docs.py` passes |
| `design/adr/` | two ADRs: the enumeration, and the mortal blow |
