...
- [x] T001 [P] [US1] Add to `tests/engine/test_rules.py`: `select_group_skill([70,45,30],
...
- [x] T002 [US1] Add to `tests/engine/test_rules.py`: `group_test(member_skills=[70,45,30],
      ...
      10 members)
- [x] T003 [P] [US2] Add to `tests/engine/test_rules.py`: `resolve_extended_interval` with a
...
- [x] T004 [US2] Add to `tests/engine/test_rules.py`: a `declaration="removes_risk"` interval
      yields `gained == 1` (the documented Assumption) without calling `roll_d100`
- [x] T005 [P] [US1] Add to `tests/engine/test_verbs.py`: `verbs.group_test(...)` returns the
      shape from data-model.md
- [x] T006 [P] [US2] Add to `tests/engine/test_verbs.py`:
      `verbs.resolve_extended_interval(...)` returns the shape from data-model.md
- [x] T007 [P] [US1] Add to `tests/engine/test_client.py`: `describe --name group-test` matches
...
- [x] T008 [P] [US2] Add to `tests/engine/test_client.py`: `describe --name
...
- [x] T009 [US1] Implement `select_group_skill(member_skills: list[int | None], mode: str) ->
...
- [x] T010 [US1] Implement `group_test(member_skills, mode, opponent, seed=None,
...
- [x] T011 [US2] Implement `resolve_extended_interval(skill, opponent, progress, target,
...
- [x] T012 [P] [US1] Add `group-test` to `TOOLS` in `engine/wyrd/catalog.py`, matching
      contracts/cli.md
- [x] T013 [P] [US2] Add `extended-task-interval` to `TOOLS` in `catalog.py`, matching
      contracts/cli.md
- [x] T014 [P] [US1] Implement the `group_test` verb wrapper in `engine/wyrd/verbs.py`
- [x] T015 [P] [US2] Implement the `resolve_extended_interval` verb wrapper in `verbs.py`
- [x] T016 [P] [US1] Add the `group-test` subcommand to `engine/wyrd/client.py`: `--member-skills`
...
- [x] T017 [P] [US2] Add the `extended-task-interval` subcommand to `client.py`: `--skill`,
      `--opponent`, `--progress`, `--target` required ints, `--seed` optional
- [x] T018 Add `to_text` cases for `group-test` and `extended-task-interval` results in
...
- [x] T019 Run every step of `specs/078-group-tests-extended-tasks/quickstart.md` by hand and
      confirm
- [x] T020 [P] Run `ruff check engine/ tests/engine/` and `ruff format --check engine/
      tests/engine/`, fix anything flagged
- [x] T021 Update the epic tracking: this is the last of #208's four children (#221, #222, #223,
