...
| Document | Change |
|---|---|
| `design/03-rules.md` §1 | "Opposed tests" rewritten to the player-facing shape (generalised from §2's wording); the two-player-controlled-entities carve-out restated explicitly as its own rule |
| `design/03-rules.md` §2 | unchanged in substance; its "Combat does not use this shape" cross-reference to §1 is revisited since §1 no longer describes a different shape |
| `design/adr/0016-*.md` | left untouched (accepted ADRs are never edited) |
| `design/adr/0035` (new) | supersedes ADR 0016 in full (pending Step 1's grep confirming no live scope survives) |
| `design/README.md` | updated only if a new document is added (none planned — this is a rewrite of an existing document plus one ADR) |
...
**Scale/Scope**: one section of `design/03-rules.md` rewritten in place, one new ADR, one check
script
