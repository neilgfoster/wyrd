...
- [ ] T001 Write `tools/check_gear.py`, mirroring `tools/check_bestiary.py`'s shape: parse a
...
- [ ] T002 In the same script, validate `class` (weapon) as exactly `casual`/`martial` and
  `price` as a non-negative number on every entry; report every failing entry and field, not just
  the first. [FR-003]
- [ ] T003 Write `specs/023-standing-material-economy/example-gear.yaml`: a small worked example
...
- [ ] T004 Write `specs/023-standing-material-economy/example-gear-broken.yaml`: a copy with one
...
- [ ] T005 Add **Standing** to the engine-label table at the top of `design/03-rules.md`,
...
- [ ] T006 In `design/03-rules.md` (near the other tracks, §4-5's neighbourhood), state what
  raises and lowers Standing in play, and cross-reference `design/10-diegesis.md` for how it is
  rendered to the player (diegetic status, never a raw number). [FR-001, FR-002]
- [ ] T007 Rewrite `design/04-session.md:112`'s Upkeep line in place: "lose 1 Standing, or spend
...
- [ ] T008 Expand `design/13-authoring-a-setting.md:19`'s `gear.yaml` line into the field list
  ...
  FR-004, SC-002]
- [ ] T009 In `design/03-rules.md` §2, add the wealth (coin) definition: a small stated count, no
...
- [ ] T010 In `design/10-diegesis.md`'s "Inventory — realistic, not logistic" section, extend the
  ...
  [FR-006, SC-004]
- [ ] T011 In `design/03-rules.md` §2, next to the existing casual/martial sentence, state the
...
- [ ] T012 Write `design/adr/0033-standing-and-the-material-economy.md`: Standing kept and
...
- [ ] T013 Add ADR 0033 to `design/README.md`'s index.

## Phase 6 — Verification

- [ ] T014 `grep -rn "Standing" design/` and confirm every hit resolves within a document that
  also defines the term — no remaining dangling reference. [SC-001]
- [ ] T015 Read `design/04-session.md`'s Upkeep step cold, with no other document open; confirm
  every term it uses resolves within `design/`. [SC-003]
- [ ] T016 `grep` the touched files for setting/system vocabulary and tonal register; confirm
  none. [FR-008]
- [ ] T017 Run `python3 tools/check_gear.py specs/023-standing-material-economy/example-gear.yaml`
  — must pass. [SC-002]
- [ ] T018 Run `python3 tools/check_docs.py` — must pass.
- [ ] T019 Run `python3 tools/backlog.py check` — must pass.
