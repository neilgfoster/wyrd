...
**Independent Test**: Read `docs/design/16-session.md`'s Upkeep step cold, with no other document
...
- **SC-002**: A setting author can write a complete, valid `gear.yaml` weapon and armour entry
  ...
  example.
- **SC-003**: The Upkeep step in `docs/design/16-session.md` requires no document outside `design/` to
  fully resolve.
