...
- [ ] T001 Create `tests/engine/fixtures/corpus_excerpt/` with a small real-shaped corpus tree:
...
- [ ] T002 Create `engine/wyrd/corpus_excerpt.py` with module docstring explaining why this is a
...
- [ ] T003 In `corpus_excerpt.py`, implement `read_excerpt(documents_index, setting, setting_dir,
...
- [ ] T004 [US1] In `tests/engine/test_corpus_excerpt.py`, test: a known doc+offset returns text
  containing the expected phrase from T001's fixture.
- [ ] T005 [US1] [P] In the same file, test: calling `read_excerpt` twice with identical arguments
  returns identical strings (determinism, FR-005).
- [ ] T006 [US1] [P] In the same file, test: a record that exists but belongs to a different
...
- [ ] T007 [US1] [P] Against a real setting fixture (skip gracefully if not present — see note
...
- [ ] T008 [US2] Test: unknown `doc` id returns `None`, no exception (FR-004).
- [ ] T009 [US2] [P] Test: `offset >= len(text)` for a real document returns `None` (FR-004).
- [ ] T010 [US2] [P] Test: negative `offset` returns `None` (FR-004, Edge Cases).
- [ ] T011 [US2] [P] Test: a `documents.json` record whose corpus text file is missing on disk
  returns `None` rather than raising `FileNotFoundError` (FR-004, Edge Cases).
- [ ] T012 [US2] [P] Test: `offset=0` is treated as a valid start-of-document offset, not as
  falsy/missing (Edge Cases) — returns the first `window` characters, not `None`.
- [ ] T013 [US2] [P] Test: `window=0` returns a valid (possibly empty) string, not `None` and not
...
- [ ] T014 [US3] In `engine/wyrd/client.py`, add a `find` subparser with `noun`/`rule`/`table`
...
- [ ] T015 [US3] In `client.py`, add `_run_find_noun`/`_run_find_rule`/`_run_find_table`: load
  ...
  behavior).
- [ ] T016 [US3] Wire the three new `_run_find_*` functions into `main()`'s verb dispatch, matching
  the existing `elif args.verb == "..."` pattern.
- [ ] T017 [US3] [P] Add `tests/engine/test_client_find.py` (or extend an existing client test
...
- [ ] T018 [P] Run `ruff check .` and `ruff format --check .` repo-wide; fix anything flagged
  (CLAUDE.md — must stay clean, including this feature's own new files).
- [ ] T019 [P] Update `docs/design/26-corpus-index.md`'s "Retrieval" section to note the excerpt
...
- [ ] T020 Run `PYTHONPATH=engine python3 -m unittest discover tests/engine -v` and
