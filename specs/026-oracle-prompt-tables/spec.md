...
- Q: Should oracle prompt tables be documented as a variant of the existing 'oracles' family row
  in `design/03a-tables.md`'s index, or get their own separate index row? → A: Variant of oracles
  — one `Oracles` index row covers both `design/03a-5-oracle-answers.md` and the new
  `design/03a-6-oracle-prompts.md`, the same way criticals already holds several variant tables
...
override mechanism from `design/03a-tables.md` and `design/13-authoring-a-setting.md` accepts,
...
- What happens when a generated NPC objective conflicts with a companion's already-established
  objective or Tension (`design/04-session.md`)? The document states that a roll never overrides
...
- What happens when the GM has a prompt table available but doesn't roll it? As with answer
  ...
  consistent with `design/01-principles.md`.
...
- **FR-002**: Each prompt family MUST define at least one table, addressed by a key, following the
  row schema and roll-declaration conventions of [`design/03a-tables.md`](../../design/03a-tables.md).
...
- **FR-005**: The document MUST state which class of situation obliges the GM to roll a prompt
  table rather than invent, consistent with `design/01-principles.md`, mirroring how
  `design/03a-5-oracle-answers.md` states the answer-oracle obligation.
- **FR-006**: The document MUST state how a family's generated content maps onto existing content
  ...
  [`design/04-session.md`](../../design/04-session.md), a thread-turn row onto the thread/threat
  structures in [`design/05-campaign.md`](../../design/05-campaign.md) and
  [`design/15-arcs-and-beats.md`](../../design/15-arcs-and-beats.md) — so a roll's output is
...
- **FR-007**: The document MUST specify the setting extension path: a setting adding its own rows
  ...
  mechanism in `design/03a-tables.md` and [`design/13-authoring-a-setting.md`](../../design/13-authoring-a-setting.md).
- **FR-008**: The document MUST state what a prompt roll records to state and where, consistent
  with the recording conventions in `design/03a-tables.md`, so a generated result can be reused
  rather than regenerated if the same gap is asked again.
- **FR-009**: `design/03a-tables.md`'s index row for oracles MUST be amended to also link
  `design/03a-6-oracle-prompts.md`, the same way the row already links
  `design/03a-5-oracle-answers.md` — prompts are a variant of the existing oracles family, not a
  sixth index row, per the Clarifications above.
- **FR-010**: `design/02-architecture.md` and `design/07-tooling.md` MUST be updated if the
...
- **Prompt table**: a rollable table within a family, following `design/03a-tables.md`'s row
...
- **SC-003**: Grepping `design/` for setting or system vocabulary introduced by this change returns
  nothing.
- **SC-004**: A setting author reading only this document and `design/13-authoring-a-setting.md`
  can correctly state how to add a table-specific row without replacing the engine's table.
- **SC-005**: A second GM reading only `design/01-principles.md` and this document, with no other
...
- The prompt families are a closed, small set chosen for where invention runs loose in the
  existing design (companion objectives in `design/04-session.md`; thread/threat turns in
  `design/05-campaign.md` and `design/15-arcs-and-beats.md`), not an open-ended content library —
  actual setting-specific prompt content is explicitly out of scope (issue #21) and belongs in a
  `wyrd-<setting>` repo.
- The document lives at `design/03a-6-oracle-prompts.md` per the issue's stated goal, as a sibling
  to `design/03a-5-oracle-answers.md` under the same `oracles` family named in
  `design/03a-tables.md`'s index — resolved in Clarifications above as a variant of that one
  family, matching how criticals already hold several variant tables under one family entry.
- Prompt tables are `repeatable` (per `design/03a-tables.md`'s uniqueness declaration), matching
...
- Each prompt table declares its own die and modifier, per `design/03a-tables.md`'s "declared by
...
- No verification script is assumed necessary the way `tools/check_oracle_answers.py` checks #20's
  ...
  planning if a computable property (e.g. row-range contiguity per `design/03a-tables.md`) turns
