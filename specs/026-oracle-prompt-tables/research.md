...
presented-vs-actual state, `docs/design/18-campaign.md`/`docs/design/28-arcs-and-beats.md`'s thread turns,
...
**Rationale**: `docs/design/07-tables.md`'s naming convention is `<family>-<variant>` where a family
...
die, which breaks `docs/design/07-tables.md`'s rule that a family declares its own die: the four
...
**Rationale**: As with `docs/design/12-oracle-answers.md`, `docs/design/03-rules.md` §1 already commits
...
a second die vocabulary purely for this family (`docs/design/07-tables.md`'s "declared by the family"
...
**Rationale**: Mirrors the answer-oracle's obligation shape (`docs/design/12-oracle-answers.md`),
...
`docs/design/12-oracle-answers.md` rejects it: an oracle nobody is obliged to consult constrains
...
transition the thread/threat tracking in `docs/design/18-campaign.md` and `docs/design/28-arcs-and-beats.md`
...
**Rationale**: `docs/design/26-authoring-a-setting.md` already names two distinct override shapes:
...
tables. This feature closes that gap the way `docs/design/26-authoring-a-setting.md`'s own "not
...
kind of table change `docs/design/07-tables.md`'s own versioning section already names, as distinct
from *tuning*).

**Alternatives considered**: A prompt-specific extension mechanism, scoped to this family only —
rejected. `docs/design/26-authoring-a-setting.md`'s override set is closed and generalised on purpose;
