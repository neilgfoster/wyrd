...
Expected: for each of the four `oracle-prompt-*` tables in `design/03a-6-oracle-prompts.md`,
prints its row count and confirms ranges are contiguous, start at 1, and the last row is open at
the top (`design/03a-tables.md`'s row-schema rule); confirms every row carries the `checked` extra
...
recorded check; read `design/03a-6-oracle-prompts.md` by eye against a grim example and a comic
...
grep -rniE '(d&d|dnd|pathfinder|call of cthulhu|blades in the dark|mythic|fate core|apocalypse world)' \
  design/03a-6-oracle-prompts.md design/03a-tables.md design/13-authoring-a-setting.md
```
...
Expected: exits 0 — `design/03a-6-oracle-prompts.md` is reachable from `README.md` via
`design/03a-tables.md`'s index, which already links every family in the table.
...
1. Pick one example situation per family (an NPC whose objective isn't established; a scene that
   ...
   `design/03a-6-oracle-prompts.md` states plainly that the GM is obliged to roll rather than
   invent in each case.
2. Roll (or pick) a row from each of the four tables and confirm its `effect` maps onto a field
   `design/04-session.md` or `design/05-campaign.md` / `design/15-arcs-and-beats.md` already
   defines, with no new state structure invented to hold it.
3. Read `design/13-authoring-a-setting.md`'s amended `extend:` override and confirm a
