...
beyond what `docs/design/07-tables.md` already fixes for every family; this file names the entities
...
- **Keys**: `oracle-prompt-npc-objective`, `oracle-prompt-situation-truth`,
  ...
  `oracles` family entry in `docs/design/07-tables.md`'s index, alongside the sibling `oracle-answer`
  key.
...
- **File**: none yet — like all five other table families, the tables exist only as rows in their
  design document (`docs/design/13-oracle-prompts.md`) until Stage 13's engine implementation reads
...
| `checked` | the family's declared extra field: records that this row was read once grim, once comic, and passed both (`docs/design/07-tables.md`'s "a family may declare further fields" clause) |
...
(`docs/design/07-tables.md`'s versioning section; `docs/design/19-state.md`'s log-provenance shape), plus
...
- `subject`: what the roll is generating content *for* — an NPC's name, a thread's name, a scene's
  label — the same free-text identification discipline `docs/design/12-oracle-answers.md` already
...
(`docs/design/16-session.md`), a thread's state (`docs/design/18-campaign.md` /
`docs/design/28-arcs-and-beats.md`) — rather than in a second, parallel store; the beat log entry above
...
combined table's now-last row stays open at the top, per `docs/design/07-tables.md`'s convention.
