...
beyond what `design/03a-tables.md` already fixes for every family; this file names the entities
...
- **Keys**: `oracle-prompt-npc-objective`, `oracle-prompt-situation-truth`,
  ...
  `oracles` family entry in `design/03a-tables.md`'s index, alongside the sibling `oracle-answer`
  key.
...
- **File**: none yet — like all five other table families, the tables exist only as rows in their
  design document (`design/03a-6-oracle-prompts.md`) until Stage 13's engine implementation reads
...
| `checked` | the family's declared extra field: records that this row was read once grim, once comic, and passed both (`design/03a-tables.md`'s "a family may declare further fields" clause) |
...
(`design/03a-tables.md`'s versioning section; `design/06-state.md`'s log-provenance shape), plus
...
- `subject`: what the roll is generating content *for* — an NPC's name, a thread's name, a scene's
  label — the same free-text identification discipline `design/03a-5-oracle-answers.md` already
...
(`design/04-session.md`), a thread's state (`design/05-campaign.md` /
`design/15-arcs-and-beats.md`) — rather than in a second, parallel store; the beat log entry above
...
combined table's now-last row stays open at the top, per `design/03a-tables.md`'s convention.
