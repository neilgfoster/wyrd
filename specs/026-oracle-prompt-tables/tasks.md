...
- [X] **T001** Write `tools/check_oracle_prompts.py`: for each of the four `oracle-prompt-*`
  tables in `docs/design/15-oracle-prompts.md`, assert row ranges are contiguous, start at 1, and
...
- [X] **T002** Write `docs/design/15-oracle-prompts.md`:
  ...
  - The four prompt families with stated rationale for the set (NPC objective, situation truth,
    ...
    (`docs/design/16-session.md`, `docs/design/19-campaign.md` / `docs/design/18-arcs-and-beats.md`).
  - The roll declaration block per family (`key`, `die`, `modifier`, `uniqueness`, `extra row
    fields`), matching the format used in `docs/design/14-oracle-answers.md`.
  ...
  - The "what a setting may replace/extend" section, referencing the amended `extend:` mechanism
    from T004.
- [X] **T003** Amend `docs/design/04-tables.md`:
  ...
  - "What a setting may replace" section: add the additive-extension path (append rows above the
    ...
    `tables:` replacement.
- [X] **T004** Amend `docs/design/24-authoring-a-setting.md`: `extend:` gains tables as an extendable
...
- [X] **T005** Check `docs/design/02-architecture.md:91` and `docs/design/27-tooling.md:84` against the
