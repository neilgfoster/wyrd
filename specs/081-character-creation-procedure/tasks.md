...
- [ ] T001 Create `tests/engine/test_creation.py`: for each of `mortality` `"low"`,
...
- [ ] T002 Add to `tests/engine/test_creation.py`: across at least 3 differently-shaped valid
      inputs, `stamina` is always `{current: 6, max: 6}` and `taint`/`trauma`/`strain`/`dread`
      are always `0`, `resolve` is `{current: 0}` (SC-002)
- [ ] T003 Add to `tests/engine/test_creation.py`: a valid career + 8-advance allocation produces
...
- [ ] T004 Add to `tests/engine/test_creation.py`: an allocation `career.validate_allocation`
      ...
      afterward
- [ ] T005 Add to `tests/engine/test_creation.py`: `name`, `loyalty`, `drives`, `misfortune`,
      ...
      `{score: 0, label: null}`
- [ ] T006 Add to `tests/engine/test_creation.py`: a created character round-trips through
...
- [ ] T007 [P] Add to `tests/engine/test_verbs.py`: `verbs.create_character(...)` returns the
      documented shape for both an accepted and a rejected allocation
- [ ] T008 [P] Add to `tests/engine/test_client.py`: `describe --name create-character` matches
...
- [ ] T009 Create `engine/wyrd/creation.py`: `MORTALITY_FATE = {"low": 2, "standard": 3, "high":
      4}`, `STARTING_STAMINA = 6`
- [ ] T010 Implement `create_character(path, name, career, actions, loyalty, mortality,
...
- [ ] T011 [P] Add `create-character` to `TOOLS` in `engine/wyrd/catalog.py`, matching
      contracts/cli.md
- [ ] T012 [P] Implement the `create_character` verb wrapper in `engine/wyrd/verbs.py`
- [ ] T013 [P] Add the `create-character` subcommand to `engine/wyrd/client.py`: parse
...
- [ ] T014 Add a `to_text` case for `create-character` results in `engine/wyrd/render.py`
...
- [ ] T015 Run every step of `specs/081-character-creation-procedure/quickstart.md` by hand and
      confirm, including inspecting the actual on-disk file after both a valid and a rejected
      creation
- [ ] T016 [P] Run `ruff check engine/ tests/engine/` and `ruff format --check engine/
      tests/engine/`, fix anything flagged
- [ ] T017 With this PR, #210 (Character creation)'s two children (#231, #232) are both closing —
