...
- [ ] T001 Write `tools/check_advancement.py`: model a character completing careers
...
- [ ] T002 In the same script, find and assert the Stamina value at which a further +1 stops
...
- [ ] T003 In the same script, scan the skill spread across the 10+ completions and assert no
...
- [ ] T004 Run the script and capture its output (the cap, the Stamina sequence, and the
  ceiling) for the design document. [SC-001, SC-004]

## Phase 2 — Design document

- [ ] T005 Update `design/03-rules.md` §6 in place: state the 70% career cap applied uniformly to
  every skill a career grants (FR-001, FR-002, FR-003), the illegal-advance rule (FR-004), no
  changelog language. [US1]
- [ ] T006 In the same section, state the completion trigger (every granted skill at the cap,
...
- [ ] T007 State the computed Stamina ceiling from T002, and cross-reference
  `03c-character-creation.md`'s starting-value reasoning rather than restating it. [US3, FR-009]
- [ ] T008 Confirm the companion carve-out already in §6 ("no career graph, no Marks") is left
  intact and unambiguous against the new player-character rules just added. [FR-011]

## Phase 3 — Decision record

- [ ] T009 Write `design/adr/0032-career-cap-and-the-stamina-ceiling.md`: the flat-70%-cap
...
- [ ] T010 Add ADR 0032 to `design/README.md`'s index.

## Phase 4 — Verification

- [ ] T011 `grep` the touched files for setting/system vocabulary and tonal register; confirm
  none. [FR-012]
- [ ] T012 Re-read the companion carve-out (T008) and the "harder to replace, not harder to
  kill" claim in §6 against the new text; confirm neither is contradicted.
- [ ] T013 Run `python3 tools/check_docs.py` — must pass. [SC-005]
- [ ] T014 Run `python3 tools/backlog.py check` — must pass. [SC-005]
