...
table (`design/03a-4-afflictions.md`): the test at 6+ Trauma, a table large enough to survive a
...
`design/03-rules.md` §5 specifies the Affliction *track* precisely (test on every Trauma point at
...
at a Taint threshold) is already resolved: `design/03a-3-transformations.md` (#18, merged) states
...
- **FR-007**: `design/03-rules.md` §5 MUST be updated in place to match, without changelog
  language.
- **FR-008**: `design/03a-tables.md`'s index MUST carry the affliction family's row, no longer
