...
- [x] T001 Create `tests/engine/test_career.py`: each of the four worked spreads from
...
- [x] T002 Add to `tests/engine/test_career.py`: total != 8 is rejected naming the total
      ...
      cases (SC-002)
- [x] T003 Add to `tests/engine/test_career.py`: an ancestry-granted skill (not in the career) is
...
- [x] T004 [P] Add to `tests/engine/test_verbs.py`: `verbs.validate_allocation(...)` returns the
      documented shape for both an accepted and a rejected allocation
- [x] T005 [P] Add to `tests/engine/test_client.py`: `describe --name validate-allocation`
...
- [x] T006 Create `engine/wyrd/career.py`: `effective_cap(skill, career, ancestry=None) -> int |
      None` (the higher of the career's and ancestry's cap for `skill`, `None` if in neither)
- [x] T007 Implement `validate_allocation(actions: list[dict], career: dict, ancestry: dict |
      ...
      depends on T006
- [x] T008 [P] Add `validate-allocation` to `TOOLS` in `engine/wyrd/catalog.py`, matching
      contracts/cli.md
- [x] T009 [P] Implement the `validate_allocation` verb wrapper in `engine/wyrd/verbs.py`
- [x] T010 [P] Add the `validate-allocation` subcommand to `engine/wyrd/client.py`:
...
- [x] T011 Add a `to_text` case for `validate-allocation` results in `engine/wyrd/render.py`
...
- [x] T012 Run every step of `specs/080-career-advance-allocation/quickstart.md` by hand and
      confirm
- [x] T013 [P] Run `ruff check engine/ tests/engine/` and `ruff format --check engine/
