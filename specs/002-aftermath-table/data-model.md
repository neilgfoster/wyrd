...
does not exist yet — but each structure is what `doc/design/19-state.md` will carry.

---

## 1. The Aftermath table itself

A member of the table families defined in `doc/design/07-tables.md`. It declares:
...
This feature gives structure to the `wounds: []` list that `doc/design/19-state.md` already declares on
...
| `from.table` / `from.beat` | provenance. `doc/design/07-tables.md` already requires a table roll to record the key it rolled on; a wound outlives the log entry, so it carries its own. |
...
record is a load error rather than a row quietly ignored (`doc/design/07-tables.md`):

| `effect` | Applies |
|---|---|
| `stamina_max: -N` | permanent reduction to maximum Stamina |
| `skill: -N` | a penalty to the skill the wound bears on, applied to the *skill* and never to the roll (`doc/design/03-rules.md` §1) |
...
field when a rule reads one is additive under `doc/design/22-evolution.md`.

**Rendering**: diegetic, per `doc/design/23-diegesis.md` — "the knee never set right", never `skill: -10`.
...
Neither is new machinery; both are shapes `doc/design/27-entities.md` already fixes.
...
the chronicle carries until it resolves; a thread is the type `doc/design/27-entities.md` already has for
exactly that.
...
| `trauma` | player character frontmatter | **unchanged by this feature.** `doc/design/03-rules.md` §5 already awards 1 Trauma per critical taken; awarding more here would double-count the same blow. |

---

## 6. What is recorded

Per `doc/design/07-tables.md`, a table roll records the key it rolled on alongside the engine version:
...
what was rolled, and `doc/design/03-rules.md` §3 already says that choosing *not* to spend is itself a
