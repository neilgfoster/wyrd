...
`docs/design/03-rules.md` defers all combat death to a table that does not exist:
...
**Why this priority**: This is the sentence `docs/design/03-rules.md` already makes and cannot currently
...
**Why this priority**: It is one of the five shapes `docs/design/03-rules.md` promises, and it is the one
...
1. **Given** a death result and a spent Fate point, **When** the outcome resolves, **Then** the
   ...
   `docs/design/03-rules.md` section 3.
...
3. **Given** a character with no Fate, or a player who declines to spend it, **When** a death result
   comes up, **Then** the death stands — per the GM contract in `docs/design/01-principles.md`.
...
**Why this priority**: `docs/design/03-rules.md` and `docs/design/01-principles.md` both make load-bearing
...
- **`mortality: low` or similar settings.** `docs/design/01-principles.md` says the tone contract governs
...
- **FR-008**: The table MUST carry at least one row for each of the five outcome shapes
  `docs/design/03-rules.md` names: a permanent wound, a new enemy, capture, a disfigurement that
...
- **FR-020**: The document MUST state what the tone contract's `mortality` value changes about how
  this table is applied, honouring the claim `docs/design/01-principles.md` already makes.
...
- **FR-027**: A decision recorded here that rejected a workable alternative someone would plausibly
  propose again MUST earn an ADR in `docs/adr/`.
...
- **SC-001**: A reader given only the new document and a dropped combatant can reach a single
  ...
  nominal top.
- **SC-002**: All five outcome shapes named in `docs/design/03-rules.md` are covered by at least one row
