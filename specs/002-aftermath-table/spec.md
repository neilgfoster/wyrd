...
design document, conforming to the conventions established by #15 (`design/03a-tables.md`), so that
...
`design/03-rules.md` defers all combat death to a table that does not exist:
...
(`design/01-principles.md`), the bootstrap question "how lethal?" (`design/16-chronicle-bootstrap.md`),
the setting-authoring conversion surface (`design/13-authoring-a-setting.md`) — points at a table
...
fate to. `design/06-state.md` already carries a `wounds: []` field on the player character with no
schema behind it.
...
- Q: Issue #16 names `design/03a-1-aftermath.md`, but the index #15 merged assigns `03a-1-` to
  ...
  `design/03a-2-aftermath.md`; the issue's path is stale and the issue is corrected rather than the
  index renumbered.
...
**Why this priority**: This is the sentence `design/03-rules.md` already makes and cannot currently
...
4. **Given** a resolved Aftermath roll, **When** the outcome is recorded, **Then** the record
   carries the table's key alongside the engine version, per `design/03a-tables.md`.
...
**Why this priority**: `design/06-state.md` already declares `wounds: []` with nothing defining an
...
2. **Given** a wound record, **When** it is rendered for the player, **Then** it is rendered
   diegetically per `design/10-diegesis.md` — never as a raw score.
...
**Why this priority**: It is one of the five shapes `design/03-rules.md` promises, and it is the one
...
3. **Given** a character with no Fate, or a player who declines to spend it, **When** a death result
   comes up, **Then** the death stands — per the GM contract in `design/01-principles.md`.
...
**Why this priority**: `design/03-rules.md` and `design/01-principles.md` both make load-bearing
...
2. **Given** a companion facing a death result and a player present and able to act, **When** the
   ...
   `design/03-rules.md`'s existing rule for spending Fate for someone else.
3. **Given** a companion who dies, **When** state is updated, **Then** their `status` moves to a
   value `design/06-state.md` already declares.
...
- **A "new enemy" result.** The enemy has to become something the world can act with, not a note —
  `design/14-entities.md` already fixes what a `character` with `role: nemesis` carries, including an
...
- **`mortality: low` or similar settings.** `design/01-principles.md` says the tone contract governs
...
- **FR-001**: The engine MUST define the Aftermath family in `design/03a-2-aftermath.md`, one table
...
- **FR-002**: The document MUST declare the family's roll as **`d100 + (5 × points below zero)`**,
  explicitly, per `design/03a-tables.md`'s rule that the roll belongs to the family and not to the
  engine.
...
- **FR-008**: The table MUST carry at least one row for each of the five outcome shapes
  `design/03-rules.md` names: a permanent wound, a new enemy, capture, a disfigurement that
...
- **FR-017**: Companions MUST roll on the same table, with the same modifier and the same rows. The
  ...
  the player is present, able to act, and spends Fate for them per `design/03-rules.md` section 3.
  No companion-specific rows, modifier or table may be introduced.
- **FR-018**: A "new enemy" result MUST create an entity conforming to `design/14-entities.md`, not
  a free-text note.
...
- **FR-020**: The document MUST state what the tone contract's `mortality` value changes about how
  this table is applied, honouring the claim `design/01-principles.md` already makes.

**Consistency with the rest of `design/`**

- **FR-021**: `design/03-rules.md` MUST link to the new document and MUST NOT continue to describe
  the table as though it were undefined.
- **FR-022**: The tables index in `design/03a-tables.md` MUST have its Aftermath row completed — its
...
- **FR-027**: A decision recorded here that rejected a workable alternative someone would plausibly
  propose again MUST earn an ADR in `design/adr/`.
...
- **Recurring wound**: a wound record whose effect fires at the start of every future fight rather
  than once.
- **New enemy**: a `character` entity per `design/14-entities.md`, with a role, a disposition and an
...
- **SC-001**: A reader given only the new document and a dropped combatant can reach a single
  ...
  nominal top.
- **SC-002**: All five outcome shapes named in `design/03-rules.md` are covered by at least one row
...
- **SC-005**: The Fate interaction and the companion question each resolve to exactly one reading
  when the new document and `design/03-rules.md` are read against each other.
- **SC-006**: A grep over all added and changed files returns no setting name and no system name.
- **SC-007**: `design/03a-tables.md`'s index has no remaining "not yet written" placeholder in the
...
- **The document is `design/03a-2-aftermath.md`, not `design/03a-1-aftermath.md`.** Issue #16 names
...
- **This feature is design-only.** There is no `engine/` directory in the repository yet, so
  `engine/tables/aftermath.yaml` is not created here even though `design/03a-tables.md` states where
