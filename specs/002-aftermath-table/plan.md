...
Write `design/03a-2-aftermath.md`: the Aftermath family, declared within the conventions #15
established. It is the table `design/03-rules.md` has deferred all combat death to since the ruleset
...
outcome shapes `design/03-rules.md` promises. Two things that both claim the moment a character would
...
Alongside the new document: `design/03-rules.md` loses its description of an undefined table,
`design/03a-tables.md`'s index row is completed, `design/06-state.md` gains the wound record its
...
**Primary Dependencies**: None. `design/03a-tables.md` (#15) must be merged — it is.
...
| Gate | Status |
|---|---|
| Nothing unpublishable enters the repository | **Pass.** Every row is written from the engine's own vocabulary. No source text, no quotation, no library catalogue. The five outcome shapes come from `design/03-rules.md`, not from a book. |
...
| Rule changes apply forward only | **Pass.** The document states that a result already rolled stands, per `design/09-evolution.md`. This feature adds a table; it recomputes nothing. |
| Design documents rewritten in place; ADRs never edited | **Pass.** `design/03-rules.md` is rewritten where it describes this table, with no "previously" note. The new ADR is new, not an edit. |
...
**Structure Decision**: the document lives at `design/03a-2-aftermath.md`, following the index
`design/03a-tables.md` established in #15, which assigns `03a-1-` to criticals. Issue #16's
...
**Coverage of what `design/03-rules.md` promises** — permanent wound (`lasting-wound`), new enemy
...
- **What Strain does.** Discovered while designing the recurring wound ([research.md](./research.md)
  D7): `design/03-rules.md` §5 says where Strain comes from and when it clears but never what it
