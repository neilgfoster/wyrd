...
`docs/design/03-rules.md` promises five shapes plus death. Each must be findable:
...
new document must be a label `docs/design/03-rules.md` already publishes.
...
```bash
sed -n '/Death is deferred/,/^---/p' docs/design/03-rules.md
```
...
was rolled, which is what keeps `docs/design/03-rules.md`'s natural roll rule intact.
