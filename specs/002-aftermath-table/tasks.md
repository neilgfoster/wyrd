...
Design documentation at `design/`, decision records at `design/adr/`. No source tree is touched:
...
- [x] T001 Create `design/03a-2-aftermath.md` with its title, a one-paragraph statement of what the
      ...
      what a setting may replace. Present tense, no changelog (`design/README.md`).
...
- [x] T002 [US1] Write the **family declaration** section of `design/03a-2-aftermath.md` from
      ...
      the exhaustion clause of `design/03a-tables.md` does not apply because the family is not
...
- [x] T004 [US1] Write the **table** into `design/03a-2-aftermath.md`: all eight rows, each carrying
...
- [x] T005 [US1] Add the **reading procedure and the recorded outcome**: roll, apply the modifier,
      ...
      `design/03a-tables.md`. Include the log shape from [data-model.md](./data-model.md) §6,
...
- [x] T008 [US2] Write the **lasting wound** section of `design/03a-2-aftermath.md` from
...
- [x] T009 [US2] State explicitly that the record carries **no healing field, no duration and no
      ...
      answer would prejudge it. Adding one later is additive under `design/09-evolution.md`
      (FR-013, SC-008).

- [x] T010 [US2] Amend `design/06-state.md`: give the existing `wounds: []` field its record shape,
      in place, present tense, no changelog. Cross-link to `design/03a-2-aftermath.md`. Add the
      diegetic-rendering note pointing at `design/10-diegesis.md` — a wound is "the knee never set
...
- [x] T011 [US4] Write the **closing the death rows** section of `design/03a-2-aftermath.md`: one
...
- [x] T012 [US4] State the **`mortality`** clause explicitly: `low` closes the death rows for
      ...
      `design/01-principles.md` already makes about `mortality` (FR-020).

- [x] T013 [US4] Write `design/adr/0009-fate-closes-the-death-rows.md`: the decision that Fate closes
...
- [x] T014 [US4] Confirm against `design/03-rules.md` §3 that nothing here contradicts Fate's
...
- [x] T016 [US3] State **why `−10` and not a new number**: `design/03-rules.md` §1 already publishes
...
- [x] T018 [US5] State the **state changes** for a companion outcome, using only values
      `design/06-state.md` already declares: `status: dead` on an unsaved death, `status: away` while
...
**Purpose**: two rows create things in the world; both must reuse the shapes `design/14-entities.md`
...
- [x] T019 [P] Write the **new enemy** clause: the `new-enemy` row creates a `character` entity with
      ...
      elsewhere. Link `design/14-entities.md` (FR-018).

- [x] T020 [P] Write the **capture** clause: the `taken` row opens a `thread` entity, the type
      `design/14-entities.md` already has for an open loop the chronicle carries. Say what ends it is
...
- [x] T022 State that **Trauma is unchanged** by this table: `design/03-rules.md` §5 already awards
...
- [x] T023 Amend `design/03-rules.md` §2: link `03a-2-aftermath.md`, and stop describing the table as
...
- [x] T024 Complete the **Aftermath row** of the index in `design/03a-tables.md`: roll
...
- [x] T025 Grep for **anywhere else** that describes Aftermath as undefined or future —
      `design/01-principles.md`, `design/04-session.md`, `design/07-tooling.md`,
      `design/13-authoring-a-setting.md`, `design/16-chronicle-bootstrap.md`,
      `design/09-evolution.md`. Update in place where the description is now wrong; leave alone where
...
- [x] T030 Run quickstart step 7 — read `design/03-rules.md` §2 and §3 against
      `design/03a-2-aftermath.md` and confirm each of the four interactions resolves to exactly one
...
- T010 (`design/06-state.md`) touches a different file from the rest of Phase 4.

## Implementation strategy

**MVP** is Phases 1–3: the table exists, is declared, and resolves a dropped combatant. Everything
after that closes a promise `design/03-rules.md` has already made, and R1.2 needs Phase 4.
