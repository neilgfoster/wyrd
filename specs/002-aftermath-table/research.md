...
**Why**: `doc/design/03-rules.md` already distinguishes a combatant who dropped by 1 from one who dropped
...
`doc/design/07-tables.md` already requires of every family.
...
1. At `mortality: low` the lowest possible total became `1 + 5 − 10 = −4`, so the table's first row
   no longer sat at the family's lowest possible total. `doc/design/07-tables.md` requires ranges to
...
The replacement is also a closer reading of what `doc/design/01-principles.md` actually claims:
...
**Why**: `doc/design/03-rules.md` already guarantees that a character who spends Fate "survives and is
...
- **Fate declared before the roll.** More tense, but the player commits blind, and it converts Fate
  from a death valve into a gamble — `doc/design/03-rules.md` is explicit that Fate is spent *to avoid
...
(`doc/design/03-rules.md` §3). Fate may only be spent against a **death** result; it may not be spent to
...
**Why**: the asymmetry the engine wants already exists and is already written down. `doc/design/03-rules.md`
§3 says companions have no Fate and rely on the player's; `doc/design/01-principles.md` says the GM may
...
**Why**: `doc/design/07-tables.md` is explicit that severity is family-specific and that "a field
...
`doc/design/22-evolution.md` and costs nothing to defer.
...
**Why**: `doc/design/07-tables.md` sets the test — "taking the same wound twice is ordinary, so criticals
...
**Why**: `doc/design/03-rules.md` §1 already establishes that difficulty modifies the *skill*, never the
...
- **1 Strain at the start of each fight.** Tempting — Strain is the engine's short-term pressure
  track and this is short-term pressure. Rejected because `doc/design/03-rules.md` §5 says where Strain
...
**Why**: `doc/design/03-rules.md` claims "most results are a lasting mark rather than death". `CLAUDE.md`
