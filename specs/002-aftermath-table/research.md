...
**Why**: `docs/design/03-rules.md` already distinguishes a combatant who dropped by 1 from one who dropped
...
The replacement is also a closer reading of what `docs/design/01-principles.md` actually claims:
...
**Why**: `docs/design/03-rules.md` already guarantees that a character who spends Fate "survives and is
...
- **Fate declared before the roll.** More tense, but the player commits blind, and it converts Fate
  from a death valve into a gamble — `docs/design/03-rules.md` is explicit that Fate is spent *to avoid
...
(`docs/design/03-rules.md` §3). Fate may only be spent against a **death** result; it may not be spent to
...
**Why**: the asymmetry the engine wants already exists and is already written down. `docs/design/03-rules.md`
§3 says companions have no Fate and rely on the player's; `docs/design/01-principles.md` says the GM may
...
**Why**: `docs/design/03-rules.md` §1 already establishes that difficulty modifies the *skill*, never the
...
- **1 Strain at the start of each fight.** Tempting — Strain is the engine's short-term pressure
  track and this is short-term pressure. Rejected because `docs/design/03-rules.md` §5 says where Strain
...
**Why**: `docs/design/03-rules.md` claims "most results are a lasting mark rather than death". `CLAUDE.md`
