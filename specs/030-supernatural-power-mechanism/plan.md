...
transformation-threshold path. This is [ADR 0036](../../design/adr/0036-one-configurable-power-mechanism.md)'s
...
**One schema, not a menu of shapes.** `design/13-authoring-a-setting.md`'s hard rule — a setting
...
| Document | Change |
|---|---|
| `design/03a-7-systems-of-power.md` (new) | the schema: declaration, cost, training gate, Ill Omen consequence, worked example |
| `design/adr/0036-one-configurable-power-mechanism.md` (new) | records the load-bearing fork |
| `design/README.md` | links the new document so `tools/check_docs.py` finds it reachable |
| `design/03-rules.md` | unchanged in substance — casting reuses §1 verbatim; a one-line cross-reference added at the end of §1 pointing to the new document, matching how other consequence chains are cross-referenced |
...
not something the design document's prose derives after the fact. Then `design/03a-7-systems-of-power.md`,
