...
replaces, "roughly danger 2", which design/11-corpus-index.md has quoted since it was written and
which was never computed from anything, because until now there was nothing to compute it from.

Every figure design/03-rules.md section 7 and design/11-corpus-index.md publish is asserted here,
...
1. The scaling equation is danger_effective = danger x (party_effective / written_for)
   (design/03-rules.md section 7). Its shape is not changed by this feature.
2. written_for is the head count the content was written for, a scaling input and never a gate
   (design/11-corpus-index.md). Its meaning is not changed by this feature.
3. The party is a query, not a roster: characters with role companion and status with-party,
   plus the player character (design/06-state.md). status has exactly five values.
4. danger is a multiplier inside content -- a trap written Nd4 does 6d4 at danger 6, and enemy
   counts and skill values scale from the same number (design/03-rules.md section 7).
...
# ---------------------------------------------------------------------------
# The five status values a companion can hold (design/06-state.md). Only one
...
def check_published_figures(published: dict) -> None:
    ...
    # design/03-rules.md section 7, the effective-size table.
    ...
    # design/03-rules.md section 7 and design/11-corpus-index.md, the worked
    ...
    # design/03-rules.md section 7, the lone character's ratio against a party
    # of four.
    ...
    # design/03-rules.md section 7, the worked case's ratio.
    ...
    # design/03-rules.md section 7, the retinue table.
    ...
    # design/03-rules.md section 7, what a companion buys, first against fifth.
