...
| [0051](0051-setting-specific-corpus-lives-in-the-setting-repo.md) | Setting-specific source material lives in the owning setting repo, not the shared research repo | Superseded by [ADR 0052](../0052-the-research-repository-is-retired.md) — periodical triage is per article, not per issue, so nothing is ever actually shared between settings; the research repo is retired outright |
