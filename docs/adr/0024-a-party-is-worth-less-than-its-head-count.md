...
[`03-rules.md`](../design/03-rules.md) §7 has published the engine's only scaling equation since the
ruleset was written:
...
`24-corpus-index.md` went further and quoted a *result* — a danger-3 arc written for four, run by
...
only weighting under which the figure `24-corpus-index.md` already quoted comes out right, which is
