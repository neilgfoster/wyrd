...
- **NPC objective** rows fill a companion's `objective.wants`
  ([`16-session.md`](16-session.md)), or the equivalent unstated motive the GM is about to give
...
per `07-tables.md`'s convention. As with replacement, no extension row may carry a setting's
