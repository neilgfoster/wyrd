...
([`../13-authoring-a-setting.md`](../13-authoring-a-setting.md)). Since Wyrd resolves on
...
[`../07-tooling.md`](../07-tooling.md) exists to prevent — and the GM will forget them
...
requirement Wyrd cannot negotiate away ([`../01-principles.md`](../01-principles.md)).

**Character cost.** Wyrd models characters asymmetrically: only the player's carries full
state ([`../06-state.md`](../06-state.md)). Adopting a source game wholesale would multiply
...
- Settings must declare **conversion rules**, so conversion is repeatable rather than
  improvised each time ([`../13-authoring-a-setting.md`](../13-authoring-a-setting.md)).
