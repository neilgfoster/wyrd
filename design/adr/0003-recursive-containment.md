...
that hold their own kind to any depth ([`../14-entities.md`](../14-entities.md)). `scale` is a
...
- Entry and exit conditions exist at **every** level, so thread-matching can select a whole
  campaign or one situation out of it ([`../15-arcs-and-beats.md`](../15-arcs-and-beats.md)).
