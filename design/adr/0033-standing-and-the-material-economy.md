...
`04-session.md`'s Upkeep step charged "lose 1 Standing, or spend coin equal to Standing" with no
...
Separately, `13-authoring-a-setting.md` had promised `gear.yaml` — "weapons, armour, prices, what
...
4. How encumbrance is handled, consistent with `10-diegesis.md`'s existing rejection of a numeric
...
`10-diegesis.md` already answers the adjacent question — what a character has, and what's
...
**Wealth as a full itemized ledger.** Rejected: directly contradicts `10-diegesis.md`'s
...
capacity). Rejected on two grounds: `10-diegesis.md` already explicitly rules out an encumbrance
...
- `04-session.md`'s Upkeep line resolves entirely within `design/` — no dangling reference.
- `13-authoring-a-setting.md` gives a setting author the `gear.yaml` schema and its validator, the
  same way it already does for `bestiary.yaml`.
- `10-diegesis.md`'s "realistic, not logistic" framing is extended, not contradicted, by stating
