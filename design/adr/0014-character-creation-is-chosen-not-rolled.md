...
**Two characters of the same starting career are mechanically identical.** This is the decision's
sharpest edge and it is intended. Differentiation comes from the Drive, the Bond, the place, and
everything that then happens — not from a better roll or a cleverer allocation at a table the player
sits at once. It also means no character begins the chronicle already behind, which matters when
there is exactly one player character and no party to carry them
([`01-principles.md`](../01-principles.md)).

**Creation cannot be optimised, because there is nothing to optimise.** Consistent with the absence
of talent trees and feats, which [ADR 0002](0002-source-material.md) records as deliberate.
...
**A pool of points to distribute across skills.** Gives the player a lever and turns creation into an
optimisation exercise before the fiction has started. It also needs a costing table the engine cannot
write, since it has no idea whether a setting's skills are broad or narrow
